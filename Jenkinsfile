                ...
                steps {
                    script{
                        ...
                        withCredentials([file(credentialsId: KUBE_CREDS_ID, variable : 'KUBE_CONFIG_FILE')]){
                            ...
                            sh "sed -i 's|image: \"${env.HARBOR_URL}/${env.HARBOR_PROJECT}/${env.IMAGE_NAME}:.*\"|image: \"${env.HARBOR_URL}/${env.HARBOR_PROJECT}/${env.IMAGE_NAME}:${env.IMAGE_TAG}\"|' ${env.DEPLOYMENT_YAML}"
